...
export class GoogleDoc {
    ...
    constructor() {
        let credentials = ""
        try {
            credentials = fs.readFileSync('credentials.json').toString()
        } catch (err) {
            throw new Error("Cannot read credentials.json - required to use Google Docs writer");
        }
    ...
    async create(docName: string): Promise<string> {
        ...
        if (!documentId) throw Error("Could not create new document");
    
        console.log(`Document is available at \nhttps://docs.google.com/document/d/${documentId}`);
    ...
    public async addEntry(documentId: string, {timestamp, author, text}: DocEntry) {
        const client = google.docs({ version: 'v1', auth: this.auth });
    ...
    private formatEntry({timestamp, author, text}: DocEntry): docs_v1.Schema$Request[] {
